...
<#macro pull_request_comment_link><@repo_link/>: ${markdown.link('PR #' + event.pullRequest.id + ': ' + event.pullRequest.title, 'https://git.example.com/projects/' + event.pullRequest.toRef.repository.project.key + '/repos/' + event.pullRequest.toRef.repository.slug + '/pull-requests/' + event.pullRequest.id + '/overview?commentId=' + event.comment.id?url)}</#macro>
