...
${markdown.bold(event.comment.author.displayName)} (edited<#if event.comment.author.displayName != event.actor.displayName>by ${markdown.escape(event.actor.displayName)}</#if>): ${markdown.escape(event.comment.text)}
